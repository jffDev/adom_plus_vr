...
def plot_all_losses(
    n_workers,
    grad_loss_adom=[], com_loss_adom=[],
    grad_loss_adom_vr=[], com_loss_adom_vr=[],
    graph_type=None,
    id_max=None,
    compression=0.0,
):
    ...
    for i in range(2):
        ...
        for k, loss_list in enumerate(loss_list_bis):
            ...
            if len_list > 0:
                ...
                if k == 0:
                    data_dict["Methods"] = data_dict["Methods"] + ["ADOM+"] * len(x_axis)
                elif k == 1:
                    data_dict["Methods"] = data_dict["Methods"] + ["ADOM+VR"] * len(x_axis)
...
def plot_all_losses_test(
    ...
    Parameters:
        ...
        - graph_type (str): the type of graph considered.
        - id_max (int): the maximum index considered for the plots.
        - compression (float): in [0, 100], percentage of data to discard for the plots, in order to speed up the plotting process.
    """
    loss_list_grad = [
        grad_loss_adom,
        grad_loss_adom_vr
