...
def create_data(n_samples, dim, n_workers, classification, dataset_name):
    """
    ...
    X_data, y_data = data[0].toarray(), data[1]
    n_data = y_data.shape[0] // n_workers

    train_dataset = torch.utils.data.TensorDataset(torch.tensor(X_data), torch.tensor(y_data))
    data = torch.utils.data.DataLoader(dataset = train_dataset, batch_size=n_data, shuffle=True, drop_last = True)

    for X, y in data:
        # # compute the n_data_per_worker
        # if classification:
        #     # create a random dataset for the task of binary classification
        #     X, y = shuffle(X_data, y_data, n_samples = n_data)
...
def create_sample_data(n_samples, dim, n_workers, classification):
    """
    Create a collection of n_workers different random datasets,
    either for the task of binary classification or linear regression.
